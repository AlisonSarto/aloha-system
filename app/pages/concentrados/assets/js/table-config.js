...
function newTable() {
  ...
  //? Se não for a tabela de concentrados, não executa o script
  if ((content === undefined || content != 'concentrados') && tableHasClass == false) {
    return;
  }
  ...
  $.ajax({
    ...
    success: function(data) {
      ...
      concentrados = data.concentrados;
      concentrados.forEach(db => {
        tableContent += `
          <tr>
            <td>${db.sabor}</td>
            ...
            <td>
              <div class="d-flex flex-nowrap" data-id="${db.id}" data-name="${db.sabor}">
                <button class="btn btn-sm btn-warning mx-2 edit-concentrado"><i class="fas fa-pen-to-square"></i></button>
                <button class="btn btn-sm btn-success mov-concentrado"><i class="fas fa-arrow-right-arrow-left"></i></button>
              </div>
            </td>
          </tr>
        `;
  ...
  function returnEmptyTable() {
    tableContent += `
      <tr>
        <td colspan="3" class="text-center">Nenhum concentrado encontrado</td>
      </tr>
    `;
    dataTable(tableContent);
  }
