$(document).on('click', '.delete-situacao', function() {
  var id = $(this).parent().data('id');
  $this = $(this);
  situacao = $this.parent().data('name');

  //? Tempo de espera em s
  contador = 1;

  //* Mostra modal de confirmação
  $('#modal .modal-title').html('Deletar situação');
  ...
  $('#modal .modal-body').html(`
    <p>
      Deseja realmente deletar a situação <b>${situacao}</b>? <br>
      <b>Obs:</b> as entradas não serão deletadas.
    </p>
  `);
  $('#modal').modal('show');
  ...
  $('#delete').click(function() {
    ...
    $.ajax({
      url: `/api/situacoes/delete?id=${id}`,
      type: 'DELETE',
      beforeSend: function() {
