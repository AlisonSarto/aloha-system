$(document).on('click', '.delete-grupo', function() {
  ...
  $this = $(this);
  grupo = $this.parent().data('name');

  //? Tempo de espera em s
  contador = 1;

  //* Mostra modal de confirmação
  $('#modal .modal-title').html('Deletar Grupo de Situação');
  ...
  $('#modal .modal-body').html(`
    <p>
      Deseja realmente deletar o grupo <b>${grupo}</b>? <br>
      <b>Obs:</b> Não é possivel deletar um grupo que tenha situações vinculadas.
    </p>
  `);
  $('#modal').modal('show');
  ...
  $('#delete').click(function() {
    ...
    $.ajax({
      url: `/api/grupos-situacoes/delete?id=${id}`,
      type: 'DELETE',
      beforeSend: function() {
